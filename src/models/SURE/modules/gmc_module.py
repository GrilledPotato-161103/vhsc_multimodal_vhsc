...
from models.SURE.modules.gmc_baseline import TransformerEncoder
...
class AffectJointProcessor(nn.Module):
    def __init__(self, common_dim, dataset='mosei', transfer=False):
        ...
        # else:
        #     # Language
        #     self.proj_l = nn.Conv1d(1, 30, kernel_size=1, padding=0, bias=False)
        #     self.trans_l_with_a = get_affect_network(self_type='la', layers=5)
        #     self.trans_l_with_v = get_affect_network(self_type='lv', layers=5)
        #     self.trans_l_mem = get_affect_network(self_type='l_mem', layers=5)

        #     # Audio
        #     self.proj_a = nn.Conv1d(1, 30, kernel_size=1, padding=0, bias=False)
        #     self.trans_a_with_l = get_affect_network(self_type='al', layers=5)
        #     self.trans_a_with_v = get_affect_network(self_type='av', layers=5)
        #     self.trans_a_mem = get_affect_network(self_type='a_mem', layers=5)

        #     # Vision
        #     self.proj_v = nn.Conv1d(1, 30, kernel_size=1, padding=0, bias=False)
        #     self.trans_v_with_l = get_affect_network(self_type='vl', layers=5)
        #     self.trans_v_with_a = get_affect_network(self_type='va', layers=5)
        #     self.trans_v_mem = get_affect_network(self_type='v_mem', layers=5)
    ...
    def forward(self, x):
        ...
        # if self.transfer:
        #     x_l = x_l.max(dim=1, keepdim=True).values
        #     x_a = x_a.max(dim=1, keepdim=True).values
        #     x_v = x_v.max(dim=1, keepdim=True).values
...
class AffectJointProcessor2(nn.Module):
    def __init__(self, common_dim, dataset='mosei', transfer=False):
        super(AffectJointProcessor2, self).__init__()
        ...
        if dataset == 'mosei':
            ...
            #Language
            self.proj_l = nn.Conv1d(60, 30, kernel_size=1, padding=0, bias=False)
            self.trans_l_with_a = get_affect_network(self_type='la', layers=5)
            # self.trans_l_with_v = get_affect_network(self_type='lv', layers=5)
            self.trans_l_mem = get_affect_network(self_type='l_mem', layers=5)

            # Audio
            self.proj_a = nn.Conv1d(60, 30, kernel_size=1, padding=0, bias=False)
            self.trans_a_with_l = get_affect_network(self_type='al', layers=5)
            # self.trans_a_with_v = get_affect_network(self_type='av', layers=5)
            self.trans_a_mem = get_affect_network(self_type='a_mem', layers=5)
        ...
        self.projector = nn.Linear(60, common_dim)
    ...
    def forward(self, x):
        ...
        #     x_v = x_v.max(dim=1, keepdim=True).values
        # Project the textual/visual/audio features
        proj_x_l = self.proj_l(x_l.unsqueeze(2))
        proj_x_a = self.proj_a(x_a.unsqueeze(2))
        # proj_x_v = self.proj_v(x_v)
        proj_x_a = proj_x_a.permute(2, 0, 1)
        proj_x_l = proj_x_l.permute(2, 0, 1)
        # proj_x_v = proj_x_v.permute(2, 0, 1)
...
class AffectGRUEncoder(nn.Module):
...
class AffectEncoder(nn.Module):
    ...
    def __init__(self, common_dim, latent_dim, positive=False):
    ...
    def forward(self, x):
        if self.positive:
            return torch.log(1 + torch.exp(self.encode(x)))
        else:
            x = self.layer_norm(self.encode(x))
        return x