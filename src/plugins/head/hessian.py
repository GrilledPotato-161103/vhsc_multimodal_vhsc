...
from torch.func import functional_call, vmap, jacrev, hessian, jacfwd
import rootutils
...
class HessianBiModalInferer(nn.Module):
    def __init__(self,
    ...
    def inference_mode(self):
        # 1. Record the initial state (True if training, False if eval)
        originally_training = self.training
        try:
            # 2. Switch to eval mode for inference
            self.eval()
            yield
        finally:
            # 3. This block ALWAYS runs, even if errors occur during 'yield'
    ...
    def get_predictor_fn(self):
        def infer(z):
            return self.predictor(z)
        return infer
    
    def get_jacobian_hessian_fn(self, recon_fn, pred_fn):
        def single_pred(z_single):
            z_recon = recon_fn(z_single.unsqueeze(0))
            pred = pred_fn(z_recon)
            return pred.squeeze()
    ...
    def forward(self, z: torch.Tensor, sigma_z: torch.Tensor, pred: torch.Tensor, signal: tuple = (1, 1)):
        ...
        # Hessian forbids randomness induction inside procedure
        with self.inference_mode():
            recon_fn, pred_fn = self.get_recon_fn(signal=signal), self.get_predictor_fn()
            jac_fn, hess_fn = self.get_jacobian_hessian_fn(recon_fn, pred_fn)
            J = jac_fn(z)
            H = hess_fn(z)
            # Compute sigma_pred_sq from e2e for alpha prediction
            sigma_pred_sq = torch.einsum("bi,bij,bj->b", J, sigma_z, J).unsqueeze(-1)
        ...
        w_H = torch.bmm(H, sigma_z)
        numerator = torch.linalg.vector_norm(w_H.flatten(start_dim=1), ord=2, dim=1)
        grad_norm_sq = torch.sum(J ** 2, dim=1)
        # compute trace
        trace_sigma = torch.diagonal(sigma_z, dim1=1, dim2=2).sum(dim=1)
        kappa = numerator / (grad_norm_sq * trace_sigma + self.eps)
        kappa = kappa.unsqueeze_(-1)
        # mode == "learned" — bounded heads (see __init__ notes).
        output_latent = self.output_enc(pred)
        # print(J.shape, sigma_z.shape, sigma_pred_sq.shape, output_latent.shape)

        beta_raw = self.beta_net(torch.cat([output_latent, kappa], dim=-1))
        # Normalization
        ...
        ekf_feat = torch.log(torch.cat([output_latent, torch.sqrt(sigma_pred_sq)], dim=-1).clamp_min(self.eps))
        # Test if propagate loss 
        inv_alpha = self.inv_alpha_net(ekf_feat) + self.eps
        mu = self.mu_head(output_latent)
        return mu, inv_alpha, beta, sigma_pred_sq
    
class DiscreteHessianBiModalInferer(nn.Module):
    def __init__(self,
                    reconstructor: nn.Module,
                    predictor: nn.Module,
                    latent_dim: 16,
    ...
    def forward(self, z: torch.Tensor, sigma_z: torch.Tensor, pred: torch.Tensor, signal: tuple = (1, 1)):
        ...
        einvals, einvecs = torch.linalg.eigh(sigma_z)
        v_principal = einvecs[..., -1]
        delta = self.eps * v_principal
        z_plus, z_minus = z + delta, z - delta
        pred_plus, pred_minus, pred = pred_fn(recon_fn(z_plus)), pred_fn(recon_fn(z_minus)), pred if pred is not None else pred_fn(recon_fn(z))
        numerator = torch.abs(pred_plus + pred_minus  - 2 * pred)
        denominator = torch.abs(pred_plus - pred_minus)
        kappa = numerator / (denominator + self.eps)
        # 1. Project Sigma_z onto the principal direction to get directional variance
